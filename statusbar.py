...
class StatusBar:
    def __init__(self, screen, ship_x, ship_y, settings, bullets_num, aliens_num, ship_limit):
        self.screen = screen
        ...
        self.text_surface = self.text_font_obj.render(self.text_contents
                                                      % (ship_x, ship_y, bullets_num, aliens_num, ship_limit),
                                                      True,
                                                      self.text_color)
        self.text_rect = self.text_surface.get_rect()
        self.update(ship_x, ship_y, bullets_num, aliens_num, ship_limit)

    def update(self, ship_x, ship_y, bullets_num, aliens_num, ship_limit):
        self.text_surface = self.text_font_obj.render(
            self.text_contents % (
                ship_x, ship_y, bullets_num, aliens_num, ship_limit), True,
            self.text_color)
